...
/*METTRE LE NOMBRE DE LIGNES DE L'IMAGE*/
	 int Nlig=345; /*direction des x*/
/* nombre de colonnes*/
/*METTRE LE NOMBRE DE LIGNES DE L'IMAGE*/
	 int Mcol=460; /*direction des y*/
...
*/
	...
	int code;
	MPI_File descripteur, descripteur2;
	MPI_Offset deplacement_initial, deplacement_initial2;
	MPI_Status statut;
	/* Ouverture du fichier "image_initiale_f.bin" en lecture */
	code = MPI_File_open(comm2d, "world_map.bin", MPI_MODE_RDONLY,MPI_INFO_NULL, &descripteur);
	...
	if (code != MPI_SUCCESS) {
		...
		/* Ouverture du fichier "image_initiale_level_set_function.bin" en lecture */
	code = MPI_File_open(comm2d, "image_geometrique_level_set_function.bin", MPI_MODE_RDONLY,MPI_INFO_NULL, &descripteur2);
	/* Test pour savoir si ouverture du fichier est correcte */
	if (code != MPI_SUCCESS) {
		fprintf(stderr, "ATTENTION erreur lors ouverture du fichier");
		MPI_Abort(comm2d, 2);
	...
	double * phi_local_ligne=malloc((tab_bounds[1]-tab_bounds[0]+3)*(tab_bounds[3]-tab_bounds[2]+3)*sizeof(double));
	double ** phi_local=allocarray((tab_bounds[1]-tab_bounds[0]+3),(tab_bounds[3]-tab_bounds[2]+3));

	if(nu_local==NULL)
		printf("Erreur dans l'allocation mémoire de nu_local-- \n");
	if(phi_local_ligne==NULL)
		printf("Erreur dans l'allocation mémoire de phi_local-- \n");
	...
	deplacement_initial2=0;

	MPI_File_set_view(descripteur,deplacement_initial,MPI_DOUBLE,mysubarray,"native",MPI_INFO_NULL);
	MPI_File_set_view(descripteur2,deplacement_initial2,MPI_DOUBLE,mysubarray,"native",MPI_INFO_NULL);

	MPI_File_read(descripteur,nu_local,(tab_bounds[1]-tab_bounds[0]+3)*(tab_bounds[3]-tab_bounds[2]+3),MPI_DOUBLE,&statut);
	MPI_File_read(descripteur2,phi_local_ligne,(tab_bounds[1]-tab_bounds[0]+3)*(tab_bounds[3]-tab_bounds[2]+3),MPI_DOUBLE,&statut);

	MPI_File_close(&descripteur);
	MPI_File_close(&descripteur2);
	...
	for(int i=0;i<(tab_bounds[1]-tab_bounds[0]+3);i++){
		 ...
		 phi_local[i]=&(phi_local_ligne[i*(tab_bounds[3]-tab_bounds[2]+3)]);
	}
	...
	// algo principal
	 ...
	 double eps=0.000001;
	 double c1,c2;
	 double aintum,aintun,saintum,saintun;
	 ...
	 for(k=0;k<Itermax;k++){
		 ...
		 MPI_Sendrecv(&phi_local[1][1], 1, type_colonne, voisin[W], etiquette, &phi_local[1][nb_colonne_res +1], 1, type_colonne, voisin[E], etiquette, comm2d, &status);
			...
			MPI_Reduce(&m,&sm,1,MPI_INTEGER,MPI_SUM,0,comm2d);
			MPI_Reduce(&n,&sn,1,MPI_INTEGER,MPI_SUM,0,comm2d);
			MPI_Reduce(&aintum,&saintum,1,MPI_DOUBLE,MPI_SUM,0,comm2d);
			MPI_Reduce(&aintun,&saintun,1,MPI_DOUBLE,MPI_SUM,0,comm2d);
			//printf("rang %d,aintum : %lf, saintum : %lf \n",rang,aintum,aintum);
			if (rang == 0){
				if(sm>0){
					c1 = saintum/sm;
				}
			...
			MPI_Bcast(&c1,1,MPI_DOUBLE,0,comm2d);
			MPI_Bcast(&c2,1,MPI_DOUBLE,0,comm2d);
		...
		if(voisin[N] == MPI_PROC_NULL && voisin[W] == MPI_PROC_NULL)
			phi_local[0][0] = phi_local[1][1];

		if(voisin[N] == MPI_PROC_NULL && voisin[E] == MPI_PROC_NULL)
			phi_local[0][ nb_colonne_res + 1] = phi_local[1][ nb_colonne_res ];

		if(voisin[W] == MPI_PROC_NULL && voisin[S] == MPI_PROC_NULL)
			phi_local[nb_ligne_res + 1][0] = phi_local[nb_ligne_res][1];

		if(voisin[S] == MPI_PROC_NULL && voisin[E] == MPI_PROC_NULL)
	...
	ecrire_mpi(*phi_local,ntx, nty, tab_bounds, comm2d);
